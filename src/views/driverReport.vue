<template>
    ...
    <div class="page-container">
        <div class="text-l"> 
            关键字：
            <input type="text" class="input-text Wdate mr-5" style="width:150px;" placeholder="姓名/工号/电话" v-model="searchInput">
            开始日期：
        ...
        <table class="table table-border table-bordered table-hover table-bg mt-20" >
            ...
            <tbody>
                <tr class="text-c" v-for="item in items">
                    ...
                    <td>
                        <a v-link="{ name: 'driverDetailReport'}" class="btn btn-link size-S radius" href="javascript:;" v-on:click="clickDetail(item)">订单详细</a>
                    </td>
                </tr>
            </tbody>
            </div>
    ...
    <div id="feedback" v-if="is_AJ()" style="padding-bottom:50px;">
        <div class="text-l">
            开始日期：
            <input v-model="startDateInputSpecific" type="text" class="input-text Wdate mr-5" style="width:120px;" id="J-start-date-Specific" v-bind:value="startDateInputSpecific">
            结束日期：
            <input v-model="endDateInputSpecific" type="text" class="input-text Wdate mr-5" style="width:120px;" id="J-end-date-Specific" v-bind:value="endDateInputSpecific">
        </div>
    </div>
</template>
...
<script>
    ...
    export default{
        ready () {
            ...
            // new Kalendae.Input('J-end-date-Specific', {
            //     months: 2,
            // });
            // new Kalendae.Input('J-start-date-Specific', {
            //     months: 2,
            // });
        ...
        methods: {
            ...
            feedback:function(id) {
                ...
                layer.open({
                    ...
                    yes: function () {
                        ...
                        .always((data)=> {
                            that.$route.router.go({path: '/job', replace: true})
                        })
                    }
                },)
            },
            exportData: function () {
                ...
                .always((data)=>{
                    var result = base.compareDate(this.endDateInput, this.startDateInput);
                    console.log(result);
                    if (result >= 300) {
                        layer.msg("日期范围已经超出，最多为三个月！" ,{icon:2,time:1000});
                        return
                    }
            ...
            queryAjax:function(callback=function(){}){
                ...
                var result = base.compareDate(this.startDateInput, this.endDateInput);
                console.log(result);
                if (result > 0) {
